...
import type { CreatorCampaignMeta, UgcSubmissionView } from "@/lib/campaign/ugc-query";
...
// صندوق رفع (تسليم جديد أو إعادة تسليم) — يرسل multipart إلى المسار المعطى.
function UploadBox({
  url,
  label,
  icon,
...
// معاينة المحتوى (فيديو/صورة) — مصادَقة بالكوكي، بثّ محميّ.
...
// بطاقة حقّ استخدام — قبول/رفض (شفاف: المدّة/القنوات/الأجر ظاهرة).
function UsageRightCard({ ur }: { ur: NonNullable<UgcSubmissionView["usageRight"]> }) {
  const router = useRouter();
  ...
  return (
    <div className="rounded-xl border border-primary/30 bg-primary/5 p-3">
      <div className="mb-1 flex items-center justify-between">
        <span className="text-sm font-semibold text-foreground">طلب حقوق استخدام</span>
      ...
      ) : ur.status === "ACCEPTED" && ur.endAt ? (
        <p className="mt-1 text-[11px] text-primary">سارية حتى {ur.endAt.slice(0, 10)}</p>
      ) : null}
    </div>
  );
}
            ...
            <div key={s.id} className="space-y-2 rounded-xl border border-border bg-card p-3">
              <div className="flex items-center justify-between">
                <span className="text-xs font-medium text-foreground">
                  {s.typeLabel} · {s.createdAt.slice(0, 10)}
                </span>
              ...
              {s.reviewNote ? (
                <p className="rounded-lg bg-muted/40 p-2 text-[11px] text-foreground">
                  ملاحظة العلامة: {s.reviewNote}
                </p>
              ) : null}
              {s.contentPayout != null ? (
                <p className="text-xs text-primary">
                  مستحقّ المحتوى: {formatMoney(s.contentPayout, s.currency)}
                </p>
              ) : null}
              {s.usageRightPayout != null ? (
                <p className="text-xs text-primary">
                  مستحقّ الحقوق: {formatMoney(s.usageRightPayout, s.currency)}
                </p>
              ) : null}
              ...
              {s.usageRight ? <UsageRightCard ur={s.usageRight} /> : null}
            </div>
          ))}
        </div>
      ) : (
